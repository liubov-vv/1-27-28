  ...
  return (
    <div>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {items.map((item) => (
          <Card key={item.title} className="h-full rounded-2xl border-[rgb(var(--border))] bg-[rgb(var(--surface))] p-6 shadow-[0_8px_24px_rgba(19,29,52,0.06)]">
            <div className="text-base font-semibold tracking-tight">{item.title}</div>
            <div className="mt-3 text-sm leading-6 text-[rgb(var(--text-muted))]">{item.description}</div>
          </Card>
        ))}
      </div>
    </div>
