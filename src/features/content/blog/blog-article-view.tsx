...
export function BlogArticleView() {
  ...
  const categoryLabel = article?.category ?? "Контент";

  return (
    <div className="space-y-8 py-8">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <Link href="/blog">
          <span className="text-sm font-semibold text-[rgb(var(--primary))] hover:opacity-90">Назад к блогу</span>
        </Link>
      ...
      {state === "error" ? <EmptyState title="Не удалось загрузить статью" description="Попробуйте позже." /> : null}
      {state === "empty" ? <EmptyState title="Статья не найдена" description="Проверьте ссылку или вернитесь назад." /> : null}
          ...
          <div className="space-y-6">
            <div className="space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <ContentMeta category={categoryLabel} publishedAt={article.publishedAt} readingTimeLabel="6–8 мин" />
              </div>
            ...
            <Card>
              <CardContent className="p-6">
                {sections.length ? (
                  <RichTextContent sections={sections} />
                ) : (
                  <p className="text-sm text-[rgb(var(--text-muted))]">Контент пока в подготовке.</p>
                )}
              </CardContent>
            </Card>

            <RelatedContent current={article} related={related} variant="blog" />
          </div>
